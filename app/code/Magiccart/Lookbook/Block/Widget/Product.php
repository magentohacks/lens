...
class Product extends \Magento\Catalog\Block\Product\AbstractProduct implements \Magento\Widget\Block\BlockInterface
{
    ...
    public function getLookbook()
    {
        ...
        $data = json_decode((string)$this->_lookbook->getData('config'), true);
        if($data) $this->_lookbook->addData($data);
    ...
    public function getContent($data)
    {
        if($data){
            ...
            return $this->filterProvider->getBlockFilter()->setStoreId($storeId)->filter($data);
        }
        return '';
    }
