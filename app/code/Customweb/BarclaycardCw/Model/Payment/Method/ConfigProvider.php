...
class ConfigProvider implements \Magento\Checkout\Model\ConfigProviderInterface
{
    ...
    public function __construct(
        ...
        foreach (array_keys($paymentHelper->getPaymentMethods()) as $code) {
        	if (strpos($code, 'barclaycardcw_') === 0) {
    ...
    protected function getFailureMessage(AbstractMethod $method)
    {
    	if ($this->_checkoutSession->getQuote()->getPayment()->getMethod() == $method->getCode()) {
	    	$failureMessage = $this->_checkoutSession->getBarclaycardCwFailureMessage();
	    	if (!empty($failureMessage)) {
	    		$this->_checkoutSession->unsBarclaycardCwFailureMessage();
    ...
    protected function getImageUrl(AbstractMethod $method)
    {
    	try {
    		...
    		return $this->_assetRepo->getUrlWithParams('Customweb_BarclaycardCw/images/payment/method/' . $method->getPaymentMethodName() . '.png', $params);
    ...
    {
    	return new \Customweb\BarclaycardCw\Model\Renderer\CheckoutForm($method->getCode());
    }
