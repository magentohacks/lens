...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getCountDown($_product, $layout='')
    {
        if($this->_timer==null) $this->_timer = $this->getThemeCfg('timer');
        if(!$this->_timer['enabled']) return;
        $now = strtotime("now");
        ...
        if($_product->getSpecialPrice() == 0 || $_product->getSpecialPrice() == "") return;
        $timer = strtotime($toDate) - $now;
        return ($timer > 0) ? '<count-down data-timer="' .$toDate. '">' . $layout . '</count-down>' : '';
    }
    ...
    public function getProductQty($_product)
    {
        ...
        if($type == 'grouped' || $type == 'bundle'){
            $allProducts = $_product->getTypeInstance()->getProductsToPurchaseByReqGroups($_product);
            if(is_array($allProducts)){
                $allProducts = reset($allProducts);
            }
    ...
    public function getLabels($_product)
    {
        ...
        if($newText && $this->isNew($_product)) $html .= '<span class="product-badges sticker top-left"><span class="labelnew">' . __($newText) . '</span></span>';
        ...
        if($saleLabel) $html .= '<span class="product-badges sticker top-right"><span class="labelsale">' . __($saleLabel) .'</span></span>';
        
        $lowStockText = isset($this->_labels['lowStockText']) ? $this->_labels['lowStockText'] : ''; // get in Cfg;
        $lowStockQty = isset($this->_labels['lowStockQty']) ? $this->_labels['lowStockQty'] : 0; // get in Cfg;

        if($lowStockText && $lowStockQty){
            $qtyStock = $this->getProductQty($_product);
            if($qtyStock <= $lowStockQty) $html .= '<span class="product-badges sticker top-right"><span class="labelstock">' . __($lowStockText) .'</span></span>';
        }
                    ...
                    if(is_array($values) || is_object($values)){
                        foreach ($values as $value) {
                            ...
                            if(isset($value['variable'])) {
                                $css .= ':root {';
                                    $css .= $value['variable'] .': ' . $value['color'] . ';';
                                $css .= '}';
                            }else{
