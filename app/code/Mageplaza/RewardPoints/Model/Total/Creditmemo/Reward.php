...
class Reward extends AbstractTotal
{
    ...
    public function collect(Creditmemo $creditmemo)
    {
        ...
        $itemCreditmemo          = $this->calculation->getOldRewardData($order->getCreditmemosCollection(), ['mp_reward_earn', 'mp_reward_spent', 'mp_reward_base_discount', 'mp_reward_discount'], $isAddRewardShippingEarn);
        ...
        foreach ($creditmemo->getAllItems() as $item) {
             ...
             */
            if ($this->canRefundEarn($order, $isRefundPointEarn, $mpRewardEarnAfterInvoice) && $rewardItem->getMpRewardEarn()) {
            ...
            if ($rewardItem->getMpRewardDiscount() > 0) {
                $itemDiscount     = $creditmemo->roundPrice(($rewardItem->getMpRewardDiscount() * $item->getQty()) / $rewardItem->getQty(), 'regular', true);
                $itemBaseDiscount = $creditmemo->roundPrice(($rewardItem->getMpRewardBaseDiscount() * $item->getQty()) / $rewardItem->getQty(), 'base', true);
