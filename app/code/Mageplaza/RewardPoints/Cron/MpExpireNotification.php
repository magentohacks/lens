...
class MpExpireNotification
{
    ...
    public function execute()
    {
        foreach ($this->_storeManager->getStores() as $store) {
            $beforeExpireDays = $this->helperData->getEmailHelper()->getEmailConfig('before_expire_days', $store->getId());
            if ($this->helperData->isEnabled($store->getId()) && $beforeExpireDays) {
                $transactionCollection = $this->transactionCollectionFactory->create()
                    ...
                    ->addFieldToFilter('expiration_date', ['to' => date('Y-m-d H:i:s', strtotime($this->dateTime->gmtDate()) + $beforeExpireDays * 86400)])
