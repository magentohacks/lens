...
class UpgradeSchema implements UpgradeSchemaInterface {
	...
	public function upgrade(SchemaSetupInterface $setup, ModuleContextInterface $context){
		...
		if (version_compare($context->getVersion(), '1.0.1') < 0) {
			/**
			 * Add column 'send_email' to 'customweb_barclaycardcw_transaction'
			 */
			if ($installer->getConnection()->isTableExists($installer->getTable('customweb_barclaycardcw_transaction'))) {
				$installer->getConnection()->addColumn($installer->getTable('customweb_barclaycardcw_transaction'), 'send_email',
						[
		...
		if (version_compare($context->getVersion(), '1.0.4') < 0) {
			...
			$rows = $installer->getConnection()->fetchAll(
					"select * from {$installer->getTable('sales_sequence_meta')} where entity_type = 'barclaycardcw_transaction'");
			foreach ($rows as $row) {
				$row['entity_type'] = 'barclaycardcw_trx';
				$row['sequence_table'] = $this->getSequenceName('barclaycardcw_trx', $row['store_id']);
			...
			foreach (array_keys($this->_storeManager->getStores(true)) as $storeId) {
				$oldTableName = $this->getSequenceName('barclaycardcw_transaction', $storeId);
				if ($installer->getConnection()->isTableExists($oldTableName)) {
					$newTableName = $this->getSequenceName('barclaycardcw_trx', $storeId);
