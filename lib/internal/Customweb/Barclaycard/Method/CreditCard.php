...
class Customweb_Barclaycard_Method_CreditCard extends Customweb_Barclaycard_Method_DefaultMethod {
	...
	// 		return array_merge(parent::getAliasCreationParameters($transaction), $this->getCardOnFileParameters($transaction));
	// 	}
	
	// 	public function getAliasManagerAuthorizationParameters(Customweb_Barclaycard_Authorization_Transaction $transaction, array $formData, $authorizationMethod){
	// 		return array_merge(parent::getAliasManagerAuthorizationParameters($transaction, $formData, $authorizationMethod),
	// 				$this->getCardOnFileParameters($transaction));
	// 	}
	public function getFormFields(Customweb_Payment_Authorization_IOrderContext $orderContext, $aliasTransaction, $failedTransaction, $authorizationMethod, $isMoto, $customerPaymentContext){
		...
		if (Customweb_Barclaycard_Authorization_Hidden_Adapter::AUTHORIZATION_METHOD_NAME == $authorizationMethod ||
			...
			// Set field names
			$formBuilder->setCardHolderFieldName('CN')->setCardNumberFieldName('CARDNO')->setCvcFieldName('CVC')->setExpiryMonthFieldName(
					'ECOM_CARDINFO_EXPDATE_MONTH')->setExpiryYearFieldName('ECOM_CARDINFO_EXPDATE_YEAR')->setExpiryYearNumberOfDigits(2);
			...
			if (strtolower($this->getPaymentMethodName()) == 'creditcard') {
				$formBuilder->setCardHandlerByBrandInformationMap($this->getPaymentInformationMap(),
						$this->getPaymentMethodConfigurationValue('credit_card_brands'), 'brand')->setAutoBrandSelectionActive(true);
			}
			else {
				$formBuilder->setFixedBrand(true)->setSelectedBrand($this->getPaymentMethodName())->setCardHandlerByBrandInformationMap(
						$this->getPaymentInformationMap(), $this->getPaymentMethodName(), 'brand');
			}
			...
			if ($failedTransaction !== null) {
				...
				if ($errorCode !== null) {
					$formBuilder->setExpiryElementErrorMessage($this->getExpiryErrorMessage($errorCode))->setCardHolderElementErrorMessage(
							$this->getCardHolderErrorMessage($errorCode))->setCardNumberElementErrorMessage(
							$this->getCardNumberErrorMessage($errorCode))->setCvcElementErrorMessage($this->getCVCErrorMessage($errorCode));
				}
	...
	public function getAuthorizationParameters(Customweb_Barclaycard_Authorization_Transaction $transaction, array $formData, $authorizationMethod){
		...
		return array_merge($parameters, $this->getCardOnFileParameters($transaction));
	}
	...
	public function is3DEnabled(){
		if (!$this->existsPaymentMethodConfigurationValue('threeds_state')) {
			return true;
		}
		return $this->getPaymentMethodConfigurationValue('threeds_state') == 'enabled';
	}

	private function getCardOnFileParameters(Customweb_Barclaycard_Authorization_Transaction $transaction){
		$parameters = array();
		$isAlias = false;
		if ($transaction->getTransactionContext()->getAlias() instanceof Customweb_Barclaycard_Authorization_Transaction) {
			$parameters['COF_TRANSACTION'] = 'SUBEQ'; // prioritise over new recurring alias
			$isAlias = true;
		}
		else if ($transaction->getTransactionContext()->createRecurringAlias() || $transaction->getTransactionContext()->getAlias() === 'new') {
			$parameters['COF_TRANSACTION'] = 'FIRST';
			$isAlias = true;
		}
		if ($isAlias) {
			if ($transaction->getAuthorizationMethod() === Customweb_Payment_Authorization_Recurring_IAdapter::AUTHORIZATION_METHOD_NAME) {
				$parameters['COF_INITIATOR'] = 'MIT';
			}
			else {
				$parameters['COF_INITIATOR'] = 'CIT';
			}
			$parameters['COF_SCHEDULE'] = 'UNSCHED';
		}
		return $parameters;
	}
}