		...
		this.attachListeners = function() {
			...
			$(document).on('change', formElement + ' [data-field-alias="select"]', function() {
				...
				if (!customer.isLoggedIn()) {
					serviceUrl = urlBuilder.createUrl('/guest-carts/:cartId/barclaycardcw/checkout/payment-form/:paymentMethod/:alias', {
						cartId: quote.getQuoteId(),
						paymentMethod: paymentMethod,
						alias: self.getValue() || 0
					});
				} else {
					serviceUrl = urlBuilder.createUrl('/carts/mine/barclaycardcw/checkout/payment-form/:paymentMethod/:alias', {
						paymentMethod: paymentMethod,
						alias: self.getValue() || 0
					});
				}
