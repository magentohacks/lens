...
class Spending extends AbstractTotal
{
    ...
    )
    {
        ...
        $this->calculation->resetRewardData(
            $items,
            $quote,
            ['mp_reward_base_discount', 'mp_reward_discount', 'mp_reward_shipping_base_discount', 'mp_reward_shipping_discount'],
            ...
            if ($ruleId == 'rate' && $pointSpent) {
                ...
                if ($pointSpent) {
                    ...
                    foreach ($items as $item) {
                        ...
                        if ($item->getHasChildren() && $item->isChildrenCalculated()) {
                            ...
                            foreach ($item->getChildren() as $child) {
                                $this->calculateDiscount($child, $pointSpent, $spendingTotal, $totalPointSpent, $totalBaseDiscount, $totalDiscount, $totalDiscountByRate);
                                $lastItem = $child;
                            }
                        } else {
                            $this->calculateDiscount($item, $pointSpent, $spendingTotal, $totalPointSpent, $totalBaseDiscount, $totalDiscount, $totalDiscountByRate);
                            $lastItem = $item;
                        }
                    }
                    ...
                    if ($this->calculation->isSpendingOnShippingFee($quote->getStoreId())) {
                        ...
                        $shippingDiscount     = $this->calculation->convertPrice($baseShippingDiscount, false, false, $item->getStoreId());
     ...
     * @param $totalDiscount
     */
    protected function calculateDiscount($item, $pointSpent, $spendingTotal, &$totalPointSpent, &$totalBaseDiscount, &$totalDiscount, $totalDiscountByRate)
    {
