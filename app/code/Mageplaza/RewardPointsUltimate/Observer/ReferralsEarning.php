...
class ReferralsEarning implements ObserverInterface
{
    ...
    public function execute(EventObserver $observer)
    {
        ...
        $this->calculation->resetRewardData($items, $quote, ['mp_reward_referral_earn', 'invited_earn'], ['mp_reward_referral_earn']);
        foreach ($rules as $rule) {
            ...
            if ($totalCustomer) {
                ...
                if ($lastItem && $totalRefererEarn > $mpRefererEarn) {
                    $refererLastChildEarn = $this->calculation->roundPointForReferer($rule, $totalRefererEarn, $mpRefererEarn);
                ...
                if ($totalCustomerEarn) {
                    ...
                    if ($rule->getCustomerApplyToShipping()) {
                        $mpRewardShippingEarn = $this->calculation->roundPointForCustomer($rule, $totalCustomerEarn, $mpCustomerEarn, true);
                        $quote->setMpRewardShippingEarn($quote->getMpRewardShippingEarn() + $mpRewardShippingEarn);
                    } else if ($lastItem && $totalCustomerEarn > $mpCustomerEarn) {
                        $customerLastItemEarn = $this->calculation->roundPointForCustomer($rule, $totalCustomerEarn, $mpCustomerEarn);
