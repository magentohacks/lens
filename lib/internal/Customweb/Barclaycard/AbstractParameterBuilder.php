...
abstract class Customweb_Barclaycard_AbstractParameterBuilder {
	...
	protected function get3DSecureParameters(){
		if ($this->getTransaction()->getAuthorizationMethod() != Customweb_Payment_Authorization_Moto_IAdapter::AUTHORIZATION_METHOD_NAME && $this->getPaymentMethod()->is3DEnabled()) {
			return array(
				'FLAG3D' => 'Y',
	...
	private function getEmailAddress() {
		...
		$email = $email ? $email : $this->getTransaction()->getTransactionContext()->getOrderContext()->getBillingAddress()->getEMailAddress();
		$email = $email ? $email : $this->getTransaction()->getTransactionContext()->getOrderContext()->getShippingAddress()->getEMailAddress();
		if(empty($email)){
			throw new Exception(Customweb_I18n_Translation::__("Email address is required."));
		}
		return Customweb_Core_String::_($email)->substring(0, 50)->toString();
	}
