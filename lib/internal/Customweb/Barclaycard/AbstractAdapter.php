...
abstract class Customweb_Barclaycard_AbstractAdapter implements Customweb_Barclaycard_IAdapter {
	 ...
	 *        	 		  	   	 		 
	 *
	 * @return String The base URL without any specifict intention.
	 * 	 
	 */
	...
	protected function sendMaintenanceRequest($parameters){
		return Customweb_Barclaycard_Util::sendDirectRequest($this->getMaintenanceUrl(), $parameters);
	}
	...
	protected function setTransactionAuthorizationState(Customweb_Barclaycard_Authorization_Transaction $transaction, $parameters){
		...
		if ($transaction->isAuthorized() && $transaction->getAuthorizationMethod() != Customweb_Payment_Authorization_Recurring_IAdapter::AUTHORIZATION_METHOD_NAME) {
			...
			//For Hidden the ALIAS_STOREPERMANENTLY parameter is not returned
			if (isset($authorizationParameters['ALIAS']) && $transaction->getTransactionContext()->getAlias() !== null && 
					((isset($authorizationParameters['ALIAS_STOREPERMANENTLY']) && strtoupper($authorizationParameters['ALIAS_STOREPERMANENTLY']) == 'Y')
							|| $transaction->getAuthorizationMethod() != Customweb_Payment_Authorization_Ajax_IAdapter::AUTHORIZATION_METHOD_NAME)) {
				$displayName = null;
