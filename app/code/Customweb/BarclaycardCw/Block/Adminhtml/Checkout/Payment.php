...
namespace Customweb\BarclaycardCw\Block\Adminhtml\Checkout;
...
class Payment extends \Magento\Backend\Block\Widget\Form\Container
{
	 ...
	 * @var string
	 */
	protected $_blockGroup = 'Customweb_BarclaycardCw';
	...
	protected function _construct()
	{
		...
		$this->setId('barclaycardcw_checkout_payment');

		$this->buttonList->update('save', 'label', __('Submit Order'));
		$this->buttonList->update('save', 'class', 'primary');
		$this->buttonList->update('save', 'id', 'barclaycardcw-payment-submit-button');
		...
		$this->buttonList->add(
			...
			[
				'label' => __('Cancel'),
				'class' => 'cancel',
				'id' => 'barclaycardcw-payment-cancel-button'
			]
		);
	}
	...
	public function getTransaction()
	{
		if (!($this->transaction instanceof \Customweb\BarclaycardCw\Model\Authorization\Transaction)) {
			...
			if (!($transaction instanceof \Customweb\BarclaycardCw\Model\Authorization\Transaction) || !$transaction->getId()) {
