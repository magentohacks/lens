 ...
 */
class Invoice extends TotalsAbstractRender
{
    /**
     * @var string
     */
    protected $_varPrefix = 'invoice';
    ...
    {
        $this->_registry->unregister('invoice');
        $this->_registry->register('invoice', $this->getInvoice());
        $this->_registry->unregister('current_invoice');
        $this->_registry->register('current_invoice', $this->getInvoice());
        $this->_registry->unregister('current_order');
        $this->_registry->register('current_order', $this->getOrder());

        /** @var \Magestore\Pdfinvoiceplus\Block\Adminhtml\Totals\Invoice $invoiceTotals */
        $invoiceTotals = $this->_objectManager->create('Magestore\Pdfinvoiceplus\Block\Adminhtml\Totals\Invoice');

        return $invoiceTotals->getTotals();
    }
