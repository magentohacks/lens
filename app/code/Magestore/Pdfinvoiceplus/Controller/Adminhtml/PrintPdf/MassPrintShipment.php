 ...
 */
class MassPrintShipment extends AbstractMassPrintAction
{
    ...
    {
        if ($this->getRequest()->getParam('namespace') == 'sales_order_shipment_grid') {
            return $this->filter->getCollection($this->_shipmentCollectionFactory->create());
        } else {
            ...
            foreach ($orderCollection as $order) {
                ...
                $shipmentIds = array_merge($shipmentIds, $order->getShipmentsCollection()->getAllIds());
            }
    ...
    {
        if (!$collection->count()) {
            throw  new \Magento\Framework\Exception\LocalizedException(__('You have no shipment PDF file.'));
        }
