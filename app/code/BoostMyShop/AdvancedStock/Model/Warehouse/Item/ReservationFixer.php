...
class ReservationFixer
{
    ...
    public function fixForWarehouseItem($warehouseItem)
    {
        ...
        switch($status)
        {
            ...
            case self::kUnconsistent:
                //echo "<pre>"; var_dump($warehouseItem->getData()); die();
    ...
    protected function processSubReservation($warehouseItem)
    {
        ...
        foreach($this->getPendingOrderItems($warehouseItem) as $orderItem)
        {
    ...
    public function reserveQuantity($orderItemId, $qtyToReserve)
    {
        ...
        $this->_logger->log('Reserve '.$totalReservedQty.'x items for product '.$productId.' and order #'.$extendedOrderItem->getOrderItem()->getOrder()->getIncrementId(), \BoostMyShop\AdvancedStock\Helper\Logger::kLogReservation);
    ...
    public function releaseQuantity($orderItemId, $qtyToRelease)
    {
        ...
        $this->_logger->log('Release '.$qtyToRelease.'x items for product '.$productId.' and order #'.$extendedOrderItem->getOrderItem()->getOrder()->getIncrementId(), \BoostMyShop\AdvancedStock\Helper\Logger::kLogReservation);
