...
class SpeedOptimizer extends \Magento\Framework\View\Element\Template
{
    ...
    public function beforeSendResponse(Http $response)
    {
        if( !$this->helper->getConfigModule('general/enabled') 
            ...
            || $this->request->getFullActionName() == 'firecheckout_index_index'
            || $this->request->getFullActionName() == 'onestepcheckout_index_index'
            || $this->request->getFullActionName() == 'mageworx_checkout_index_index'
        ...
        $lazyloadJs = $this->secureRenderer->renderTag(
            'script',
...
javascript
        ...
        /* 
        $body = $this->addToBottomBody($body, $lazyloadJs);
        */
    ...
    {
        if($start && !is_numeric($start)) $start = strpos( (string)$content, $start);
        $html = '';
        if( $start ){
            $page = str_split( (string)$content, $start);
            $isFirst = true;
    ...
    {
        $minifyXmagento = $this->helper->getConfigModule('general/minify_xmagento');
        $content = preg_replace_callback(
            '~<\s*\bscript\b[^>]*>(.*?)<\s*\/\s*script\s*>~is',
            function($match) use($minify, $minifyXmagento, $deferJs){
                $scriptId = 'script_' . uniqid();
                if ($minify && trim($match[1], ' ')){
                    $this->scripts[$scriptId] =  $this->minifyJs( $match[0] );
                } else {
                    $isXmagento =
                        $this->str_contains($match[0], 'type="text/x-magento"')
                        || $this->str_contains($match[0], 'type="text/x-magento-template"')
                        || $this->str_contains($match[0], 'x-magento-init')
                        || $this->str_contains($match[0], 'type="application/ld+json"');

                    $this->scripts[$scriptId] = ($minifyXmagento && $isXmagento)
                        ? $this->minifyJs($match[0])
                        : $match[0];
                }
                if (!$deferJs) return '<script type="application/ld+json">' . $scriptId . '</script>';
                return '';
            },
            (string) $content
        );
    ...
    public function addLoading($content)
    {
        ...
        if($style){
            ...
            $content = str_ireplace('</head>', $style . $preload . '</head>', (string) $content);
        }
...
javascript;
        ...
        /* $content = $this->addToTopHead($content, $script); */
