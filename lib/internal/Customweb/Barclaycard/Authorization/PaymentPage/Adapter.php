...
class Customweb_Barclaycard_Authorization_PaymentPage_Adapter extends Customweb_Barclaycard_Authorization_AbstractAdapter 
	...
	public function isHeaderRedirectionSupported(Customweb_Payment_Authorization_ITransaction $transaction, array $formData) {
		...
		//Some payment methods have an issue with header redirection
		$paymentMethod = $this->getPaymentMethodByTransaction($transaction);
		if(!$paymentMethod->isHeaderRedirectionPossible()){
			return false;
		}
	...
	public function finalizeAuthorizationRequest(Customweb_Payment_Authorization_ITransaction $transaction) {
		...
		return Customweb_Core_Http_Response::htmRedirect($url);
