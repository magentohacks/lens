...
class SalesOrderCreditmemoSaveAfter implements ObserverInterface
{
    ...
    public function execute(EventObserver $observer)
    {
        ...
        if ($this->helperData->isRestorePointAfterRefund($creditmemo->getStoreId())) {
             ...
             */
            if (($mpRewardEarnAfterInvoice || (!$mpRewardEarnAfterInvoice && (in_array($order->getState(), [Order::STATE_COMPLETE, Order::STATE_CLOSED]))))) {
            ...
            $pointTransaction = $this->helperData->getTransactionByFilter(
                ['action_code' => HelperData::ACTION_SELL_POINTS_REFUND], true, false, ['field' => 'increment_id', 'value' => $order->getIncrementId()], true
            );
