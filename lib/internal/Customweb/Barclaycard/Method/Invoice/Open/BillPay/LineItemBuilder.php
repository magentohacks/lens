...
class Customweb_Barclaycard_Method_Invoice_Open_BillPay_LineItemBuilder extends Customweb_Barclaycard_AbstractLineItemBuilder {
	...
	public function build() {
		...
		// Sending only one item is not possible. As such we skip it totally.
		if (count($items) > 1) {
	...
	protected function formatAmount($amount) {
		$decimalPlaces = Customweb_Util_Currency::getDecimalPlaces($this->getOrderContext()->getCurrencyCode());
