...
final class Customweb_Barclaycard_Util {
	...
	private static function sendRequest($url, $parameters){
		...
		if($response->getStatusCode() != 200) {
			throw new Exception(
					'Barclaycard Server response is: ' . $response->getStatusCode() . ' ' . $response->getStatusMessage());
		}
	...
	public static function createBreakoutResponse($url){
		$response =  Customweb_Core_Http_Response::_(
			...
			<noscript>
				<a class="button btn barclaycard-continue-button submit" href="' . $url . '"
	...
	public static function cleanMandateIdExtended($mandateId){
		$mandateId = preg_replace("#[^A-Za-z0-9\-\?\:]+#", '', $mandateId);
		return substr($mandateId, 0, 35);
	}
