    ...
    public function getRule()
    {
        ...
        if ($rule->getReferralType() == ReferralActions::TYPE_PRICE) {
            $this->setReferMessage(
                __('You will receive %1 for every %2 amount of price purchasing made by your friends.',
                    $pointHelper->format($rule->getReferralPoints()), $rule->getReferralMoneyStep())
            );
        } else {
            $this->setReferMessage(
        ...
        switch ($customerAction) {
            ...
            case CustomerActions::TYPE_PERCENT :
                $this->setCustomerMessage(
                    __('Your friends will receive %1 percent discount on the order total when purchasing at our store', $rule->getCustomerDiscount())
                );
                break;
        }
