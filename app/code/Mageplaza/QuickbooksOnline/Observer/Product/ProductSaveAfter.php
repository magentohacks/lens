 ...
 */
namespace Mageplaza\QuickbooksOnline\Observer\Product;
 ...
 * @package Mageplaza\QuickbooksOnline\Observer\Product
 */
class ProductSaveAfter extends AbstractQueue
{
    ...
    public function executeAction(Observer $observer)
    {
        ...
        if ($product->getQuickbooksEntity() && !$product->hasQueueSave()) {
            $origData = $product->getOrigData();
            $this->helperSync->updateObject($origData, $product, QuickbooksModule::PRODUCT);
        } elseif (!$product->isObjectNew() && !$product->hasQueueSave()) {
            $this->helperSync->addObjectToQueue(QuickbooksModule::PRODUCT, $product);
        }
    }
}
