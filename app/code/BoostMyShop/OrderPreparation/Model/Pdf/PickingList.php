...
class PickingList extends \Magento\Sales\Model\Order\Pdf\AbstractPdf
{
    ...
    protected $_product;
    protected $_barcode;
    protected $_displaySummary = true;
    ...
    protected function addSummaryPage($orders)
    {
        ...
        {
            foreach ($orderInProgress->getAllItems() as $item) {
                if (!isset($items[$item->getproduct_id()])) {
                    ...
                    $obj->setorders([]);
                    $items[$item->getproduct_id()] = $obj;
                }
                $items[$item->getproduct_id()]->setipi_qty($items[$item->getproduct_id()]->getipi_qty() + $item->getipi_qty());
            }
        }
     ...
     * @param $order
     */
    protected function _drawProduct($item, $page)
    {
        switch($this->_config->getPdfPickingLayout())
        {
            case 'small':
                ...
                $lines[0][] = ['text' => $item->getSku(), 'feed' => 200, 'align' => 'left'];
            ...
            case 'large':
                ...
                $page->drawText($item->getSku(), 300, $this->y + 5, 'UTF-8');
                $page->drawText($item->getName(), 300, $this->y - 15 + 5, 'UTF-8');
