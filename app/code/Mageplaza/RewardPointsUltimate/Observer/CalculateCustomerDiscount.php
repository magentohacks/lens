...
class CalculateCustomerDiscount implements ObserverInterface
{
    ...
    public function execute(EventObserver $observer)
    {
        ...
        $fields  = ['mp_reward_invited_base_discount', 'mp_reward_invited_discount', 'mp_reward_shipping_invited_base_discount', 'mp_reward_shipping_invited_discount'];
        $this->calculation->resetRewardData($items, $quote, $fields, ['mp_reward_invited_base_discount', 'mp_reward_invited_discount']);
        ...
        foreach ($rules as $rule) {
            if (in_array($rule->getCustomerAction(), [CustomerActions::TYPE_FIXED_DISCOUNT, CustomerActions::TYPE_PERCENT])) {
                ...
                if ($discountAmount) {
                    foreach ($items as $item) {
                        ...
                        if ($item->getHasChildren() && $item->isChildrenCalculated()) {
                            ...
                            foreach ($item->getChildren() as $child) {
                                $this->calculation->calculateItemDiscount($child, $totalMathRule, $discountAmount, $baseDiscount, $discount);
                            }
                        } else {
                            $this->calculation->calculateItemDiscount($item, $totalMathRule, $discountAmount, $baseDiscount, $discount);
                        }
                    }
                     ...
                     */
                    if ($rule->getCustomerApplyToShipping()) {
                        $baseShippingDiscount = ($this->calculation->getShippingTotalForDiscount($quote, false) / $totalMathRule) * $discountAmount;
                        $baseShippingDiscount = $this->calculation->roundPrice($baseShippingDiscount, 'base_refer');
                        $shippingDiscount     = $this->calculation->convertPrice($baseShippingDiscount, false, false, $quote->getStoreId());
