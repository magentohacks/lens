...
define([
	...
	'Customweb_BarclaycardCw/js/checkout',
	'Customweb_BarclaycardCw/js/authorizationMethod',
	'Customweb_BarclaycardCw/js/alias'
], function(
	$,
	Component,
	mageTemplate,
	...
	return Component.extend({
		defaults: {
			template: 'Customweb_BarclaycardCw/payment/barclaycardcw_americanexpress',
			fieldErrorTemplate: '<div for="<%- id %>" generated="true" class="mage-error" id="<%- id %>-error"><%- message %></div>',
			authorizationUrl: url.build('barclaycardcw/checkout/authorize/')
		},
		...
		getForm: function() {
			...
			if (!customer.isLoggedIn()) {
				serviceUrl = urlBuilder.createUrl('/guest-carts/:cartId/barclaycardcw/checkout/payment-form/:paymentMethod', {
					cartId: quote.getQuoteId(),
					paymentMethod: this.item.method
				});
			} else {
				serviceUrl = urlBuilder.createUrl('/carts/mine/barclaycardcw/checkout/payment-form/:paymentMethod', {
					paymentMethod: this.item.method
				});
			}
			storage.get(
