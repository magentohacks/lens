...
{
  /**
   * Fields in  which password info store after Cart2Cart migration
   */
  const FIELD_VALIDATE_METHOD = 'pwd_validate_method';
  ...
  public function beforeAuthenticate($subject, $username, $password)
  {
    try {
      ...
      if ($validateMethod = $customer->getOrigData(AccountManagement::FIELD_VALIDATE_METHOD)) {
        ...
        if ($this->encryptionHelper->passwordIsValid($store, $version, $customer->getPasswordHash(),
          $password, $customer->getOrigData(AccountManagement::FIELD_PASSWORD_SALT))
        ) {
  ...
  {
    $customerEntityTable = $this->getConnection()->getTableName('customer_entity');
  ...
  private function deleteTempAttributes($customerId)
  {
    ...
    $customerEntityVarcharTable = $this->getConnection()->getTableName('customer_entity_varchar');
    ...
    $this->getConnection()->query("
      ...
      WHERE
        ...
        AND (ea.attribute_code = " . $this->getConnection()->quote(AccountManagement::FIELD_VALIDATE_METHOD) . "
