...
class Products extends \Magento\Backend\Block\Template {
    ...
    {
        $imageHelper = $this->_imageHelper->init($item->getRelatedProduct(), 'product_listing_thumbnail');
    ...
    {
        return json_encode($this->getTransfer()->getItems()->getProductIds());
    }
    ...
    public function getBarcodesJson()
    {
        ...
        if (!empty($barcodeAttribute))
        {
            ...
            {
                if (!empty($item->getRelatedProduct()->getData($barcodeAttribute)))
                {
                    $barcodes[$item->getRelatedProduct()->getData($barcodeAttribute)] = $item->getst_product_id();
                }
            }
        }
