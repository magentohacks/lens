...
class WrongStockItemQuantity extends AbstractDiscrepencies
{
    ...
    public function run(&$results, $fix, $productId = null)
    {
        ...
        $collection = $this->_stockItemCollectionFactory->create();
        foreach($collection as $item)
        {
            ...
            if ((int)$expectedQty != (int)$item->getQty()) {
                $label = 'product_id=' . $item->getproduct_id() . ', website_id=' . $item->getwebsite_id() . ' (stored=' . $item->getQty() . ',expected=' . $expectedQty . ')';
                ...
                if ($fix)
                {
                    ...
                    $this->createStockMovementToSyncStockItemQuantity($item, $expectedQty);
                }
            }
        }
