    ...
    public function install(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if (!$installer->tableExists(self::MAGEPLAZA_REWARD_BEHAVIOR_TABLE)) {
            $table = $installer->getConnection()
                ...
                ->addColumn('email_template', Table::TYPE_TEXT, 255, [], 'Email template sent to customers for their birthdays')
                ...
                ->addIndex(
                    $installer->getIdxName(self::MAGEPLAZA_REWARD_BEHAVIOR_TABLE, ['is_active', 'sort_order', 'to_date', 'from_date']),
        ...
        $table = $installer->getConnection()->newTable(
            ...
            $installer->getFkName(self::MAGEPLAZA_REWARD_REWARD_BEHAVIOR_WEBSITE_TABLE, 'rule_id', self::MAGEPLAZA_REWARD_BEHAVIOR_TABLE, 'rule_id'),
            'rule_id',
            $installer->getTable(self::MAGEPLAZA_REWARD_BEHAVIOR_TABLE),
            'rule_id',
            Table::ACTION_CASCADE
        )->addForeignKey(
            $installer->getFkName(self::MAGEPLAZA_REWARD_REWARD_BEHAVIOR_WEBSITE_TABLE, 'website_id', 'store_website', 'website_id'),
            'website_id',
        ...
        $table = $installer->getConnection()
            ...
            ->addIndex(
                $installer->getIdxName(self::MAGEPLAZA_REWARD_REWARD_BEHAVIOR_CUSTOMER_GROUP_TABLE, ['customer_group_id']),
                ['customer_group_id']
            )
            ->addForeignKey(
                $installer->getFkName(self::MAGEPLAZA_REWARD_REWARD_BEHAVIOR_CUSTOMER_GROUP_TABLE, 'rule_id', self::MAGEPLAZA_REWARD_BEHAVIOR_TABLE, 'rule_id'),
                'rule_id', $installer->getTable(self::MAGEPLAZA_REWARD_BEHAVIOR_TABLE), 'rule_id',
                Table::ACTION_CASCADE, Table::ACTION_CASCADE
            )
            ->addForeignKey(
                $installer->getFkName(self::MAGEPLAZA_REWARD_REWARD_BEHAVIOR_CUSTOMER_GROUP_TABLE, 'customer_group_id', 'customer_group', 'customer_group_id'),
        ...
        if (!$installer->tableExists(self::MAGEPLAZA_REWARD_REFER_TABLE)) {
            $table = $installer->getConnection()
                ...
                ->addIndex(
                    $installer->getIdxName(self::MAGEPLAZA_REWARD_REFER_TABLE, ['is_active', 'sort_order', 'to_date', 'from_date']
                    ),
                    ['is_active', 'sort_order', 'to_date', 'from_date']
                )
        ...
        $table = $installer->getConnection()->newTable(
            ...
            $installer->getFkName(self::MAGEPLAZA_REWARD_REWARD_REFER_WEBSITE_TABLE, 'rule_id', self::MAGEPLAZA_REWARD_REFER_TABLE, 'rule_id'),
            'rule_id',
            $installer->getTable(self::MAGEPLAZA_REWARD_REFER_TABLE),
            'rule_id',
            Table::ACTION_CASCADE
        )->addForeignKey(
            $installer->getFkName(self::MAGEPLAZA_REWARD_REWARD_REFER_WEBSITE_TABLE, 'website_id', 'store_website', 'website_id'),
            'website_id',
            $installer->getTable('store_website'),
            'website_id',
            Table::ACTION_CASCADE
        ...
        $table = $installer->getConnection()
            ...
            ->addForeignKey(
                $installer->getFkName(self::MAGEPLAZA_REWARD_REWARD_REFER_CUSTOMER_GROUP_TABLE, 'rule_id', self::MAGEPLAZA_REWARD_REFER_TABLE, 'rule_id'),
                'rule_id', $installer->getTable(self::MAGEPLAZA_REWARD_REFER_TABLE), 'rule_id',
                Table::ACTION_CASCADE, Table::ACTION_CASCADE
            )
            ->addForeignKey(
                $installer->getFkName(self::MAGEPLAZA_REWARD_REWARD_REFER_CUSTOMER_GROUP_TABLE, 'customer_group_id', 'customer_group', 'customer_group_id'),
                'customer_group_id', $installer->getTable('customer_group'), 'customer_group_id',
                Table::ACTION_CASCADE, Table::ACTION_CASCADE
            )
        ...
        $table = $installer->getConnection()
            ...
            ->addForeignKey(
                $installer->getFkName(self::MAGEPLAZA_REWARD_REWARD_REFERRAL_GROUP_TABLE, 'rule_id', self::MAGEPLAZA_REWARD_REFER_TABLE, 'rule_id'),
                'rule_id', $installer->getTable(self::MAGEPLAZA_REWARD_REFER_TABLE), 'rule_id',
                Table::ACTION_CASCADE, Table::ACTION_CASCADE
            )
            ->addForeignKey(
                $installer->getFkName(self::MAGEPLAZA_REWARD_REWARD_REFERRAL_GROUP_TABLE, 'referral_group_id', 'customer_group', 'customer_group_id'),
                'referral_group_id', $installer->getTable('customer_group'), 'customer_group_id',
                Table::ACTION_CASCADE, Table::ACTION_CASCADE
            )
