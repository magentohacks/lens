...
class Customweb_Barclaycard_Method_Invoice_Open_BillPay extends Customweb_Barclaycard_Method_Invoice_Open_Abstract {
	...
	public function getAuthorizationParameters(Customweb_Barclaycard_Authorization_Transaction $transaction, array $formData, $authorizationMethod) {
		...
		return array_merge(
				...
				$this->getShippingAddressParameters($transaction),
				$this->getShippingCivilityParameters($transaction, $formData),
	...
	protected function getBillingCivilityParameters(Customweb_Barclaycard_Authorization_Transaction $transaction, array $formData){
		$parameters = parent::getBillingCivilityParameters($transaction, $formData);
		$parameters['ECOM_BILLTO_POSTAL_NAME_PREFIX'] = $parameters['CIVILITY'];
		return $parameters;
	}
	
	protected function getShippingCivilityParameters(Customweb_Barclaycard_Authorization_Transaction $transaction, array $formData){
		$parameters = parent::getBillingCivilityParameters($transaction, $formData);
		return array( 'ECOM_SHIPTO_POSTAL_NAME_PREFIX' => $parameters['CIVILITY']);
	}
	...
	protected function getBillingAddressParameters(Customweb_Barclaycard_Authorization_Transaction $transaction) {
		...
		$parameters['ECOM_BILLTO_POSTAL_STREET_LINE1'] = Customweb_Barclaycard_Util::substrUtf8($splits['street'], 0, 35);
	...
	protected function getShippingAddressParameters(Customweb_Barclaycard_Authorization_Transaction $transaction) {
		$parameters = array();
		$orderContext = $transaction->getTransactionContext()->getOrderContext();
		$address = $orderContext->getShippingAddress();
		$splits = Customweb_Util_Address::splitStreet($address->getStreet(), $address->getCountryIsoCode(), $address->getPostCode());
