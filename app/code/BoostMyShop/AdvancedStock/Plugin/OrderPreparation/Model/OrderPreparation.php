...
class OrderPreparation
{
    ...
    public function aroundGetItemsToShip(\BoostMyShop\OrderPreparation\Model\OrderPreparation $subject, $proceed, $order)
    {
        $items = [];

        $collection = $this->_extendedOrderItemCollectionFactory->create()->joinOrderItem()->addOrderFilter($order->getId())->addQtyToShipFilter()->addProductTypeFilter();
