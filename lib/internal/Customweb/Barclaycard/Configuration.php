...
class Customweb_Barclaycard_Configuration {
	...
	public function getBaseEndPointUrl(){
		if ($this->isTestMode()) {
			return rtrim('https://mdepayments.epdq.co.uk', '/').'/ncol/test/';
		}
		else {
			return rtrim('https://payments.epdq.co.uk', '/').'/ncol/prod/';
		}
	}

	public function getFlexCheckoutUrl(){
		$url = 'https://payments.epdq.co.uk';
		if($this->isTestMode()){
			$url = 'https://mdepayments.epdq.co.uk/';
		}
		$url = trim($url, '/');
