...
namespace Customweb\BarclaycardCw\Plugin\Sales\Block\Adminhtml\Order;
...
class View
{
	...
	public function aroundGetReviewPaymentUrl(\Magento\Sales\Block\Adminhtml\Order\View $subject, \Closure $proceed, $action)
    {
    	if ($subject->getOrder() instanceof \Magento\Sales\Model\Order
	    		...
	    		&& $subject->getOrder()->getPayment()->getMethodInstance() instanceof \Customweb\BarclaycardCw\Model\Payment\Method\AbstractMethod) {
		    ...
		    return $subject->getUrl('barclaycardcw/transaction/reviewPayment', ['action' => $action, 'transaction_id' => $transaction->getId()]);
    	} else {
    ...
    {
		$this->_coreRegistry->register('barclaycardcw_order_view', true, true);
    }
}