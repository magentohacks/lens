...
class AutoApplyCouponObserver implements ObserverInterface {
    ...
    \Magento\Framework\App\Request\Http $request, \Magento\Checkout\Model\Session $checkoutSession, \Magento\Customer\Model\Session $customerSession, \Magento\Framework\Session\Generic $genericSession, \Magento\Quote\Model\Quote $quote, \Magento\Checkout\Helper\Cart $cartHelper, \Magento\SalesRule\Model\Coupon $salesCoupon, \Magento\SalesRule\Model\Rule $salesRule, \Magento\Framework\Stdlib\CookieManagerInterface $cookieManager, CookieMetadataFactory $cookieMetadataFactory, \Magento\Framework\Message\ManagerInterface $messageManager, \Magento\Framework\App\ResponseInterface $response, \Magento\Framework\App\Response\Http $responseUrl, \Magento\Store\Model\StoreManagerInterface $storeManager, ResultFactory $resultFactory, \Solwin\Applycoupon\Helper\Data $helper, \Magento\Framework\Serialize\Serializer\Json $serialize
    ) {
    ...
    public function execute(Observer $observer) {
        ...
        if ($moduleEnable) {
            ...
            $query_params = [];
            if($this->_helper->getCurrentUrl()){
                $query_str = parse_url($this->_helper->getCurrentUrl(), PHP_URL_QUERY);
            }
            if($query_str){
                parse_str($query_str, $query_params);
            }
            unset($query_params['coupon']);
            unset($query_params['return_url']);
            $quert_stringparam = http_build_query($query_params);
            if (isset($params['quote_id'])) {
                ...
                if (is_object($quote) && $quote->getId() && $quote->getIsActive()) {
                    ...
                    $errorMsg = $this->_helper->getConfigValue(
                        'applycouponsection/applycoupongroup/error_msg'
                    );
            ...
            if (isset($discountEmail)) {
                if ($discountEmail != '') {
                    ...
                    if ($validator->isValid($discountEmail)) {
                        ...
                        if ($this->_isCouponValid($coupon)) {
                            ...
                            if ($returnUrl != 'no') {
                                header('Location: ' . $params['return_url']
                                        . '?cc=' . $params['coupon']
                                        . '&codesuccess=1&'.$quert_stringparam);
                                exit;
    ...
    protected function _isCouponValid($couponCode) {
        try {
            ...
            if (is_object($coupon)) {
                ...
                if (is_object($rule)) {
                    // $conditionsUnSerialized = $this->serialize->unserialize(
                    ...
                    if ($rule->getIsActive()) {
                        ...
                        if (empty($startDate) || ($todaydate >= $startDate) && (empty($endDate) || ($todaydate <= $endDate))) {
