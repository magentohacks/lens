<?php
  ...
  class Index extends \Magento\Framework\App\Action\Action
  {
    ...
    public function execute()
    {
		...
		$productId = 66867; // Air optix Plus Hydraglyde
		...
		$file = $directory . '/air_optix_hydraglyde.csv';
		...
		/*if (file_exists($file)) {
			...
			echo "<br/>";
			echo $spower = $data[$i][2];
			echo $sbasecurve = $data[$i][3];
			echo $sdiameter = $data[$i][4];
			echo "<br/>";
		...
		foreach ($_children as $child){
			...
			if (file_exists($file)) {
				...
				echo $ssku = $data[$i][1];
				echo "<br/>";
				if($power == $spower && $basecurve == $sbasecurve && $diameter == $sdiameter){
					echo "Inside If";
					echo "<br/>";
					$skulength = strlen($ssku);
					if($skulength == 12){
						$ssku = "0".$ssku;
					}
					echo strlen($ssku)."<br/>";
