...
class ExtendedSalesFlatOrderItem extends \Magento\Framework\Model\AbstractModel
{
    ...
    public function createFromOrderItem($order, $orderItem)
    {
        ...
        $this->setesfoi_qty_to_ship($orderItem->getSimpleQtyToShip());
        $this->save();
    ...
    {
        return $this->getOrderItem()->getSimpleQtyToShip();
    }
     ...
     * @return $this
     */
    public function updateQtyToShip($orderItem)
    {
        $qty = $orderItem->getSimpleQtyToShip();

        if ($orderItem->getparent_item_id())
        {
            $parent = $this->_orderItemFactory->create()->load($orderItem->getparent_item_id());
