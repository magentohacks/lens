...
class Calculation extends Data
{
    ...
    public function getSpendingConfiguration($quote)
    {
        ...
        if ($maxSpending = $this->getMaxSpendingPointsByRate($quote, $rate)) {
            $spendingConfig['rules'][] = [
                'id'              => 'rate',
                'label'           => __('Each of %1 gets %2 discount',
        ...
        foreach ($quote->getAllItems() as $item) {
            ...
            if ($item->getHasChildren() && $item->isChildrenCalculated()) {
                ...
                foreach ($item->getChildren() as $child) {
                    $total += $this->getItemTotalForDiscount($child, true, $isCanCalculateRewardDiscount, $isCalculateInvitedDiscount);
                }
            } else {
                $total += $this->getItemTotalForDiscount($item, true, $isCanCalculateRewardDiscount, $isCalculateInvitedDiscount);
            }
        }
        if ($this->isSpendingOnShippingFee($storeId)) {
            $total += $this->getShippingTotalForDiscount($quote, $isCanCalculateRewardDiscount, $isCalculateInvitedDiscount);
        }
     ...
     * @return mixed
     */
    public function getItemTotalForDiscount($item, $isSpending = true, $isCalculateMpDiscount = true, $isCalculateInvitedDiscount = false)
    {
        /**
     ...
     * @return float|int
     */
    public function getShippingTotalForDiscount($quote, $isCanCalculateRewardDiscount = true, $isCalculateInvitedDiscount = false, $isSpending = true)
    {
        $total = 0;
    ...
    public function getRate($direction, $websiteId = null, $customerGroupId = null)
    {
        ...
        $customerGroupId = $customerGroupId ?: $this->getAccountHelper()->getCustomerGroupId();
