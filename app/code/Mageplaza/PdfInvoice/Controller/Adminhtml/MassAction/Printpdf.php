...
class Printpdf extends Action
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            if ($subtype) {
                $this->validateCondition($collection, $subtype);
                switch ($subtype) {
                    case Type::INVOICE:
                ...
                $this->validateCondition($collection, $type);
     ...
     * @param $type
     */
    public function validateCondition($collection, $type)
    {
        foreach ($collection as $key => $order) {
            ...
            if (!$this->helperData->validateCondition($type, $orderId)) {
